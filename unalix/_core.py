...
from unalix._config import httpopt, data_min, body_redirects
...
from unalix._http import (
    add_missing_attributes,
    create_connection,
    create_cookie_jar,
    handle_redirects,
)
from unalix._utils import (
    is_private,
...
def parse_url(url):
    ...
    if scheme not in ("http", "https"):
        raise InvalidScheme(
            message=f"Expecting 'http' or 'https', but got: {scheme}", url=url
        )
...
def unshort_url(url, parse_documents=False, cookie_policy="allow_if_needed", **kwargs):
    ...
    Parameters:
        ...
        parse_documents (`bool`, *optional*):
            If True,  will also try to obtain the unshortened URL from the
            response's body.

        cookie_policy (`str`, *optional*):
            "allow_all":  will handle cookies for all requests.
            "allow_if_needed" (default):  will handle cookies only if needed.
            "reject_all":  will not handle cookies.
    ...
    while True:
        ...
        if total_redirects > httpopt.max_redirects:
            raise TooManyRedirects(
                message="The request exceeded maximum allowed redirects", url=url
            )
        ...
        try:
            ...
            raise ConnectError(message=str(exception), exception=exception, url=url)
    ...
    for file in files:
        ...
        for provider in ruleset:
            ...
            for exception in ruleset[provider]["exceptions"]:
                exceptions.append(re.compile(pattern=exception))

            for redirection in ruleset[provider]["redirections"]:
                redirections.append(re.compile(pattern=f"{redirection}.*"))
            ...
            for referral in ruleset[provider]["referralMarketing"]:
                referrals.append(
                    re.compile(
                        pattern=rf"(%(?:26|23)|&|^){referral}(?:(?:=|%3[Dd])[^&]*)"
                    )
                )

            for raw in ruleset[provider]["rawRules"]:
                raws.append(re.compile(pattern=raw))
...
def compile_redirects(files):
    ...
    for file in files:
        ...
        for rule in ruleset:
            ...
            for pattern in patterns:
                new_patterns.append(re.compile(pattern=pattern))

            for redirect in redirects:
                new_redirects.append(
                    re.compile(
                        pattern=f".*{redirect}.*", flags=re.MULTILINE | re.DOTALL
                    )
                )

            compiled_data_as_tuple.append((new_patterns, domains, new_redirects))
...
def randomize_query(query, rules):
    ...
    randomized, not_randomized, query = ({}, {}, query.split(sep="&"))
        ...
        try:
            ...
            for rule in rules:
                if rule.match(entrie):
                    ...
                    randomized.update({key: quote(os.urandom(bytes_count))})
                    break
            else:
                not_randomized.update({key: value})
...
def parse_rules(
    ...
    Usage:
      >>> from ._utils import parse_rules
