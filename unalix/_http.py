...
from unalix._config import allowed_cookies, httpopt
from unalix._exceptions import InvalidScheme
from unalix._utils import requote_uri
...
def create_connection(scheme, netloc):
    ...
    if scheme == "http":
        ...
        connection = HTTPSConnection(
            netloc, context=httpopt.ssl_context, timeout=httpopt.timeout
        )
    else:
...
def handle_redirects(url, response):
    ...
    location = requote_uri(location.encode(encoding="latin1").decode(encoding="utf-8"))
...
def create_cookie_jar(policy_type=None):
    ...
    if policy_type == "reject_all":
        ...
        policy.set_ok = lambda cookie, request: (cookie.domain in allowed_cookies)
